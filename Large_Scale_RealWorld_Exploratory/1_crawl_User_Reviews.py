...
def main():
    ...
    # set the file path
    AppRankList_path = './UserReviewData/AppRankList.csv'
    UserReviews_path = './UserReviewData/UserReviews.csv'
    Reviews_path = './UserReviewData/Reviews.csv'
    current_rank_path = './UserReviewData/current_rank.txt'
    ...
    for i in range(current_rank, len(app_list)):
        ...
        if len(reviews_all) > 0:
            ...
            # Save Reviews
            with open('./UserReviewData/UserReviews.csv', 'a', newline='', encoding='utf-8') as file:
                writer = csv.writer(file)
                for review_id, review in enumerate(reviews_all, start=1):
                    content = review['content']
                    review_created_version = review['reviewCreatedVersion']
